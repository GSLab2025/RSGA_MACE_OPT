...
def extract_config_mace_model(model: torch.nn.Module) -> Dict[str, Any]:
    if model.__class__.__name__ not in ["ScaleShiftMACE", "MACELES", "MACERSGA"]:
        return {"error": "Model is not a ScaleShiftMACE, MACERSGA, or MACELES model"}
...
def get_params_options(
    ...
    if hasattr(model, "rsga") and model.rsga is not None:
        param_options["params"].append({"name": "rsga",
                              "params": model.rsga.parameters(),
                             "weight_decay": 0.0,
                             }
                             )
