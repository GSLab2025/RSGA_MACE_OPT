...
from mace.modules.rsga import ReciprocalSpaceGatedAttention, build_rsga_batch_context
...
class MACERSGA(MACE):
    ...
    def forward(
        ...
        if data["batch"] is None:
            rsga_batch = torch.zeros(
                positions.shape[0], dtype=torch.long, device=positions.device
            )
        else:
            rsga_batch = data["batch"]

        rsga_compute_dtype = self.rsga[0].get_compute_dtype(node_feats)
        rsga_ctx = build_rsga_batch_context(
            positions=data["positions"].to(node_feats.dtype),
            cell=data["cell"].view(-1, 3, 3).to(node_feats.dtype),
            batch=rsga_batch,
            dtype=rsga_compute_dtype,
            kspace_freq=self.rsga[0].kspace_freq,
            r_cut=self.r_max,
            exact_eval_fast_path=self.rsga[0]._use_exact_eval_fast_path(),
            phase_cache_threshold=getattr(
                self.rsga[0],
                "phase_cache_threshold",
                int(os.getenv("MACE_RSGA_PHASE_CACHE_THRESHOLD", "1000000")),
            ),
            pairwise_eval_threshold=getattr(
                self.rsga[0],
                "pairwise_eval_threshold",
                int(os.getenv("MACE_RSGA_PAIRWISE_THRESHOLD", "1024")),
            ),
        )
        ...
        for i, (interaction, product) in enumerate(
            ...
            # Per-layer RS-GA with node-level LR gating preserved.
            node_feats_lr, gate_sr_lr = self.rsga[i](
                data,
                node_feats,                  # (N, F); previously node_feats_x
                rsga_ctx,
            )
            node_feats = node_feats + gate_sr_lr * node_feats_lr
